...
import { HFacing, HPacket, HPoint, HStuff } from "gnode-api";

// I had to duplicate this file from gnode-api to add the parameter `resetReadIndex` in the `parse` method to prevent it from resetting it. This is necessary in my case because I merge packets together, so I was the parser to just continue from where I leave the read index.
...
export class HFloorItem {
	...
	static parse(packet, resetReadIndex = true) {
		if (!(packet instanceof HPacket)) {
